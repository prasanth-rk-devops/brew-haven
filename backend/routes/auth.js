...
router.post('/2fa/enable', protect, async (req, res) => {
  ...
  const totp = new otpauth.TOTP({ issuer: 'BrewHaven', label: req.user.email, secret });
...
router.post('/2fa/verify-setup', protect, async (req, res) => {
  ...
  if (totp.validate({ token, window: 1 }) === null) return res.status(400).json({ message: 'Invalid code' });

  req.user.twoFactorEnabled = true;
  req.user.backupCodes = Array.from({ length: 8 }, () => Math.random().toString(36).slice(2, 10).toUpperCase());
  await req.user.save();

  res.json({ message: '2FA enabled', backupCodes: req.user.backupCodes });
});
